 ...
 * @version 0.6-20180620
 */
;(function ($, window, document) {
    var ImgUploader = function (ele, opt) {
        if (ele.data('xwbzimg')) {
            ele.data('xwbzimg').destroy();
        }
        this.$element = ele;
        this.defaults = {
            ...
            'height': '100px',
            'separator': ',',
            'error': function (err) {window.console.error(err)} // 自定义异常捕获
        };
        ...
        this.$element.data('xwbzimg', this);
    };
    ...
    ImgUploader.prototype = {
        ...
        showImgBox: function ($d, isMultiple) {
            ...
            if (hasUpBtn || !isMultiple) {
                $imgParent.find('.xwbz-img-box:last').on('click', function () {
                    clickFunc(this, me)
                });
            }
            var $canEditBox;
            ...
            $imgParent.find('.img-del-btn').on('click', function () {
                deleteFunc(this, me)
            });

            if (isMultiple) {
                $imgParent.find('.img-edit-btn').on('click', function () {
                    editFunc(this, me)
                });
            }
        },
        ...
        destroy: function () {
            ...
            me.$element.each(function (_, item) {
                var $this = $(item);
                $this.siblings('.xwbz-img-parent-box').remove();
            });
        }
    };
    // 内部方法
    var _Methods = ['destroy'];
    // 提供使用插件的入口
    $.fn.xwbzImg = function (options) {
        options = options || {};
        if (typeof options === 'string') {
            if ($.inArray(options, _Methods) < 0) {
                if (window.console) {
                    window.console.error('不支持方法：' + options);
                }
                return this;
            }
            this.each(function (_, item) {
                var uper = $(item).data('xwbzimg');
                if (!uper && window.console) {
                    window.console.error('未初始化');
                } else {
                    uper[options].apply(uper);
                }
            });
            return this;
        }

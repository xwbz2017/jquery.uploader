...
;(function($, window, document) {
    ...
    var imgBoxClickFunc = function(e, uploader){
    ...
    ImgUploader.prototype = {
        init: function () {
            ...
            var styles = '.xwbz-img-parent-box{position: relative;min-height: 100px;}' +
                ...
                '.xwbz-img-parent-box>div>div{height: 30px;width:100px;display:none;top:-105px;position:relative;}' +
                '.xwbz-img-parent-box .img-btn{flex: 1;padding: 5px;text-align: center;color: white;}' + 
                '.xwbz-img-parent-box .img-del-btn{background: red;flex: 1;}' +
                '.xwbz-img-parent-box .img-edit-btn{background: rgba(0,0,0,0.5);}';
        ...
        showImgBox: function($d, isMultiple) {
            ...
            var imgHtml;
            if (isMultiple) {
                imgHtml = '<div><img class="xwbz-img-box" src=""><div><div class="img-btn img-del-btn">✖</div><div class="img-btn img-edit-btn">✎</div></div></div>';
            } else {
                imgHtml = '<div><img class="xwbz-img-box" src=""><div><div class="img-btn img-del-btn">✖</div></div></div>';
            }
            for (var i in urlArr) {
                if (urlArr[i]) {
                    var $img = $(imgHtml);
            ...
            if (hasUpBtn) {
                $imgParent.find(".xwbz-img-box:last").on("click", function(){imgBoxClickFunc(this, me)});
            }
            var $canEditBox;
            if (hasUpBtn) {
